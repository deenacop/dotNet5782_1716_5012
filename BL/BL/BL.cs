...
namespace BL
{
    ...
    {
        internal static DalApi dal = new DalObject();
        ...
        public BL()
        {
            ...
            IEnumerable<DO.Drone> DroneListDL = dal.GetListDrone().ToList();//Receive the drone list from the data layer.
            ...
            IEnumerable<DO.Parcel> ParcelListDL = dal.GetListParcel(i => i.MyDroneID != 0).ToList();
            ParcelListDL.CopyPropertiesToIEnumerable(ParcelListBL);//convret from IDAT to IBL

            List<BaseStation> BaseStationListBL = new();
            IEnumerable<DO.Station> StationListDL = dal.GetListStation().ToList();//Receive the drone list from the data layer.
            ...
            foreach (DroneToList currentDrone in DroneListBL)
            {
                try
                {
                    ...
                    DO.Customer senderCustomer = dal.GetCustomer(parcelDO.Sender); //CustomerListDL.Find(i => i.CustomerID == ParcelListDL[index].Sender);//sender customer
                    ...
                    //מצב סוללה:
                    DO.Customer receiverCustomer = dal.GetCustomer(parcelDO.Targetid); //CustomerListDL.Find(item => item.CustomerID == ParcelListDL[index].Targetid);//found the customer that is the targetid one
                ...
                {
                    IEnumerable<DO.Parcel> deliveredParcel = dal.GetListParcel(i => i.Delivered != null);//lists of all the delivered parcels
                    IEnumerable<DO.Station> availableStations = dal.GetListStation(i => i.NumOfAvailableChargingSlots > 0);//lists of all the available stations
                    ...
                    {//פנוי
                        ...
                        DO.Customer targetid = dal.GetCustomer(deliveredParcel.ElementAt(index).Targetid); //CustomerListDL.Find(item => item.CustomerID == deliveredParcel[index].Targetid);
